...
def visualize_top_k_act_layer(layer, folder, k, vgg16_conv, vgg16_deconv):
    act_list = []
    img_paths = load_random_image_path(folder)
    for i, img_path in enumerate(img_paths):
        img_i = load_images(img_path)
        _, act_i, _ = construct_layer_pattern(img_i, layer, vgg16_conv, vgg16_deconv)
        act_list.append(act_i)
    
    sort_list_index = sorted(range(len(act_list)), key=lambda idx: act_list[idx], reverse=True)
    ...
    for idx in range(k):
        img = load_images(img_paths[sort_list_index[idx]])
        pattern, _, _ = construct_layer_pattern(img, layer, vgg16_conv, vgg16_deconv)

        i = idx // ncols
        j =  idx % ncols

        img = cv2.imread(img_paths[sort_list_index[idx]])
        img = cv2.resize(img, (224, 224))
    ...
    img_fig.savefig(f"results/imgs/Layer {layer}.jpg")
    pattern_fig.savefig(f"results/patterns/Layer {layer}.jpg")